...
// const float INFINITE_COST = std::numeric_limits<float>::max();

static const float EPSILON = 0.00001f;
static const float INFINITE_COST = std::numeric_limits<float>::max();
...
public:
	...
	struct Node
	{
		...
		uint32_t searchIndex = 0; //The last search at which it has bean generated, basically used to decide if the node need to be reseted before we use it
	...
	Pathfinder(PathfinderAdaptor& adaptor, Cost weight = 1.0f) :
		mAdaptor(adaptor),
		mWeight(weight)
	{
		generateNodes();
	...
	std::vector<DataType> search(const DataType& start,
		const DataType& end,
		std::function<DataType(const NodeId)> idToData,
		std::function<NodeId(const DataType&)> dataToId)
	{
	...
	std::vector<NodeId> search(const NodeId startId, const NodeId endId)
	{
		...
		while (!openList.empty() && nodes[endId].g > getMin().f + EPSILON)
		{
			...
			if (!mAdaptor.lineOfSight(nodes[currId].parent, currId))
			{
			...
			for (const auto neighborInfo : nodes[currId].neighbors)
			{
				...
				{
					Cost newG = nodes[newParent].g + mAdaptor.distance(newParent, neighborId);
	...
	void generateNodes()
	{
		int nodeNum = mAdaptor.getNodeCount();
		nodes.clear();
		...
		for (auto& node : nodes)
		{
			node.neighbors = mAdaptor.getNodeNeighbors(current++);
		}
	}

	PathfinderAdaptor * getAdaptor()
	{
		return &mAdaptor;
	}
...
private:
	...
	void generateState(NodeId s, NodeId goal)
	{
		if (nodes[s].searchIndex != currentSearch)
		{
			...
			nodes[s].h = mAdaptor.distance(s, goal) * mWeight;
	...
	void addToOpen(NodeId id)
	{
		...
		{
			auto index = std::find_if(openList.begin(), 
				openList.end(), 
				[&](const HeapElement& heap){ return heap.id == id; });

			auto id = index->id;
			openList.erase(index);
			HeapElement tmp = { id, nodes[id].g, nodes[id].g + nodes[id].h };
			insert_sorted(openList, tmp);
		}
