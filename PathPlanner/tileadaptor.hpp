...
public:
    ...
    TileAdaptor(const Vectori& mapSize, 
		void * mapPtr,
		const std::function<bool(const Vectori&, const void*)>& IsTraversable) :
		mMapSize(mapSize), 
		mMapDataPtr(mapPtr),
    ...
    virtual bool lineOfSight(const NodeId n1, const NodeId n2) const override
    {
        ...
        { // Move along the x axis and increment/decrement y when f >= diff.x.
            while(l1.x != l2.x)
            {
                ...
                {  // We are changing rows, we might need to check two cells this iteration.
					if (!mIsTraversable(l1 + offset, mMapDataPtr))
					{
						return false;
					}
				...
				if (f != 0 && !mIsTraversable(l1 + offset, mMapDataPtr))
				{
					return false;
				}

                // If we are moving along a horizontal line, either the north or the south cell should be unblocked.
				if (diff.y == 0 && !mIsTraversable({ l1.x + offset.x, l1.y }, mMapDataPtr) 
					&& !mIsTraversable({ l1.x + offset.x, l1.y + 1 }, mMapDataPtr))
				{
					return false;
				}
        ...
        {  //if (diff.x < diff.y). Move along the y axis and increment/decrement x when f >= diff.y.
            while (l1.y != l2.y)
            {
				...
				if (diff.x == 0 && !mIsTraversable({ l1.x, l1.y + offset.y }, mMapDataPtr)
					&& !mIsTraversable({ l1.x + 1, l1.y + offset.y }, mMapDataPtr))
				{
					return false;
				}
    ...
    virtual std::vector<std::pair<NodeId, Cost>> getNodeNeighbors(const NodeId id) const override
    {
		...
		if (pos.x != 0 && mIsTraversable({ pos.x - 1, pos.y }, mMapDataPtr))
		{
			neighbors.push_back({ posToId({ pos.x - 1, pos.y }), cost });
		}

		if (pos.y != 0 && mIsTraversable({ pos.x, pos.y - 1 }, mMapDataPtr))
		{
			neighbors.push_back({ posToId({ pos.x, pos.y - 1 }), cost });
		}

		if (pos.x != mMapSize.x - 1 && mIsTraversable({ pos.x + 1, pos.y }, mMapDataPtr))
		{
			neighbors.push_back({ posToId({ pos.x + 1, pos.y }), cost });
		}

		if (pos.y != mMapSize.y - 1 && mIsTraversable({ pos.x, pos.y + 1 }, mMapDataPtr))
		{
			neighbors.push_back({ posToId({ pos.x, pos.y + 1 }), cost });
		}
    ...
    {
        return {(int)id % mMapSize.x, (int)id / mMapSize.x};
    }
...
private:
    ...
    const std::function<bool (const Vectori &, const void*)> mIsTraversable;
};
